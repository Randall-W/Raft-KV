...
RaftNode::RaftNode(uint64_t id, const std::string& cluster, uint16_t port, const std::string& g_store)
    : port_(port),
      pthread_id_(0),
...
void RaftNode::schedule() {
  ...
  *conf_state_ = snap->metadata.conf_state;
  ...
  redis_server_ = std::make_shared<RedisStore>(this, std::move(snap_data_), port_, store_engine);
...
// 功能：启动 Raft 节点。
void RaftNode::main(uint64_t id, const std::string& cluster, uint16_t port, const std::string& g_store) {
  // 设置信号处理函数。
  ::signal(SIGINT, on_signal);
  ::signal(SIGHUP, on_signal);
  // 创建并配置 Raft 节点。
  g_node = std::make_shared<RaftNode>(id, cluster, port, g_store);
  // 启动传输服务。
