...
int main(int argc, char* argv[]) {
  GOptionEntry entries[] = {
      ...
      {"store", 's', 0, G_OPTION_ARG_STRING, &g_store, "store engine", NULL},
      {NULL}
  };
  ...
  fprintf(stderr, "id:%lu, port:%d, cluster:%s, store_engine:%s\n", g_id, g_port, g_cluster, g_store);
  ...
  kv::RaftNode::main(g_id, g_cluster, g_port, g_store);
