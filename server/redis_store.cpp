...
RedisStore::RedisStore(RaftNode* server, std::vector<uint8_t> snap, uint16_t port, std::string& store_engine)
    : server_(server),
      acceptor_(io_service_),
      next_request_id_(0) {
    if (store_engine == "sqlite"){
        db = std::make_shared<SQlite>(server->node_id());
    }
    else if (store_engine == "rocksdb"){
        db = std::make_shared<RocksDB>(server->node_id());
    }
    else if (store_engine == "hashmap")
    {
        db = std::make_shared<HashMap>();
    }
  ...
  auto cb = [this, entry] {
    ...
    switch (data.type) {
      case RedisCommitData::kCommitSet: {
        ...
        db->set(data.strs[0], data.strs[1]);
        break;
      }
