...
RawNode::RawNode(const Config& conf, const std::vector<PeerContext>& peers) {
  ...
  //检查 last_index 是否为 0。如果是，这意味着日志是空的，这是一个新的 RawNode，类似于 StartNode。
  //在这种情况下，将 Raft 对象设置为跟随者状态，并设置初始任期和投票。
  // If the log is empty, this is a new RawNode (like StartNode); otherwise it's
  // restoring an existing RawNode (like RestartNode).
  if (last_index == 0) {
    //如果 last_index 为 0，构造函数会为每个对等节点创建一个配置更改日志条目，
    //类型为 ConfChangeAddNode，并将其添加到 Raft 日志中。
    //每个配置更改日志条目包含节点的 ID 和上下文。
...
RawNode::RawNode(const Config& conf) {
  uint64_t last_index = 0;
  // 从配置的存储中获取最后一个日志索引 last_index。如果获取失败，则记录一个致命错误。
  Status status = conf.storage->last_index(last_index);
  if (!status.is_ok()) {
    LOG_FATAL("%s", status.to_string().c_str());
  }
