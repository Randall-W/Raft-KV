...
//  使用ftell获取文件指针的当前位置，在"追加加"模式下，文件指针位于文件末尾。这实际上获取了文件的大小。
    file_size = ftell(fp);
    //检查ftell是否成功。
    if (file_size == -1) {
      LOG_FATAL("ftell error %s", strerror(errno));
    }
//  使用fseek将文件指针重置到文件的开头（SEEK_SET）
...
//  使用 ftruncate 系统调用将文件截断到指定的偏移量 offset。fileno(fp) 将文件指针 fp 转换为文件描述符。
    if (ftruncate(fileno(fp), offset) != 0) {
      LOG_FATAL("ftruncate error %s", strerror(errno));
    }
//  使用 fseek 将文件指针重置到文件的指定偏移量 offset 处。
    if (fseek(fp, offset, SEEK_SET) == -1) {
      LOG_FATAL("fseek error %s", strerror(errno));
    }
// 更新 file_size 成员变量以反映新的文件大小。
    file_size = offset;
    data_buffer.clear();
  }
...
//使用 fwrite 将 data_buffer 中的数据写入文件。
// data_buffer.data() 返回缓冲区的数据指针，1 表示每个元素的大小（字节数），data_buffer.size() 表示要写入的元素数量，fp 是文件指针。
