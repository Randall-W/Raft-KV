...
class RaftNode : public RaftServer {
 public:
  static void main(uint64_t id, const std::string& cluster, uint16_t port, const std::string& g_store);

  explicit RaftNode(uint64_t id, const std::string& cluster, uint16_t port, const std::string& g_store);
